...
import id.magau.whizz.data.model.ModelResponseFile
...
class FilePresenter(val context: Context, val mView: FileContracts.View) :
    FileContracts.Presenter {
    ...
    override fun loadData(idProduct: String) {
        ...
        mService.getFile(mToken,idProduct).apply {
            enqueue(object : Callback<ModelResponseFile> {
                override fun onFailure(call: Call<ModelResponseFile>, t: Throwable) {
                    mView.showLoading(false)
                    mView.showError(0, "Internal Server Error")
                }

                override fun onResponse(
                    call: Call<ModelResponseFile>,
                    response: Response<ModelResponseFile>
                ) {
                    mView.showLoading(false)
